...
public class ChannelManager implements MessageHandler {
	...
	private final void waitForGlobalSuccessOrFailure() throws IOException {
		...
		try {
			synchronized (channels) {
				while ((globalSuccessCounter == 0)
						&& (globalFailedCounter == 0)) {
					if (shutdown) {
						throw new IOException(
								"The connection is being shutdown");
					}
		...
		try {
			while (len > 0) {
				...
				synchronized (c) {
					...
					thislen = (c.remoteWindow >= len) ? len
							: (int) c.remoteWindow;
	...
	public void requestExecCommand(Channel c, String cmd, String remoteEncoding)
			throws IOException {
		...
		synchronized (c.channelSendLock) {
			...
			if (remoteEncoding != null) {
				tm.sendMessage(sm.getPayload(remoteEncoding));
			} else {
		...
		try {
			...
			synchronized (c) {
				...
				while (true) {
					...
					/*
					 * Do not wait if more data will never arrive (EOF or
					 * CLOSED)
					 */
				...
				if (!extended) {
					...
					System.arraycopy(c.stdoutBuffer, c.stdoutReadpos, target,
							off, copylen);
					...
					System.arraycopy(c.stderrBuffer, c.stderrReadpos, target,
							off, copylen);
			...
			/*
			 * If a consumer reads stdout and stdin in parallel, we may end up
			 * with sending two msgWindowAdjust messages. Luckily, it does not
			 * matter in which order they arrive at the server.
			 */
